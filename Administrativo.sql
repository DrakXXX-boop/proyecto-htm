...
SELECT
  ...
  fa.modelo AS marca,
  CAST(fa.capacidad_kw AS CHAR),
  IFNULL(fa.capacidad_kw,0),
  fa.estado,
  fa.fecha_instalacion
FROM administrativo a
JOIN fuentes_alternas fa ON a.id_admin = fa.id_admin;

-- --------- 9) SELECTS PARA CONSULTAR TODO ---------
...
DESCRIBE cultura_digital;
DESCRIBE mecatronica;
DESCRIBE mantenimiento;
DESCRIBE fuentes_alternas;

-- Ver contenido de cada tabla
SELECT * FROM administrativo;
SELECT * FROM cultura_digital;
SELECT * FROM mecatronica;
SELECT * FROM mantenimiento;
SELECT * FROM fuentes_alternas;

-- Ver inventario completo unificado
SELECT * FROM inventario_general ORDER BY area, categoria;

-- Contar items por categoría
SELECT categoria, SUM(cantidad) AS total_items
